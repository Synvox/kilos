...
const transformKeys = (obj, method = 'camel') => {
  if (typeof method === 'string') method = methods[method];

  if (typeof obj !== 'object') return obj;
  if (!obj) return obj;
  if (Array.isArray(obj)) return obj.map(item => transformKeys(item, method));

  return Object.keys(obj).map(key => ({ key, value: transformKeys(obj[key]) })).map(({ key, value }) => ({
    value,
    key: key.replace(/_|\-/g, ' ').replace(/(\b|^|[a-z])([A-Z])/g, '$1 $2').replace(/ +/g, ' ').trim().toLowerCase().split(' ').reduce((str, word, index) => str + method(word, index), key.startsWith('_') ? '_' : '')
